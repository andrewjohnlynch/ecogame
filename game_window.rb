class GameWindow < Gosu::Window
  ...
  def initialize
    ...
    @carnivores = add_to_window(Carnivore, CARNIVORE_START_COUNT)
    @font = Gosu::Font.new(TEXT_SIZE, {name: 'default'})
  end

  def add_to_window(klass, count)
    #initializes the arrays of entities
  ...
  def update_carnivores
    @carnivores.reject! do |carnivore|
      carnivore.eat(@herbivores)
 #     carnivore.eat(@carnivores)
      carnivore.move
      carnivore.dead?
    end
    @carnivores = @carnivores.map(&:multiply).flatten
  end
  ...
  def new_food_count
    (@foods.count * NEW_FOODS_MULTIPLIER).to_i + 1
  end
  ...
  def draw_menu
    ...
    @font.draw('Carnivores', 10, 60, ZOrder::TEXT)
    @font.draw_rel(@carnivores.count, MENU_WIDTH - 10, 60, ZOrder::TEXT, 1, 0)
